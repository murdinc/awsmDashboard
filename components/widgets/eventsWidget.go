...
func (e EventsWidget) Render() gr.Component {
	...
	// Current
	e.BuildEventsTable("Current Events", events, true).Modify(widget)

	// Recent
	e.BuildEventsTable("Recently Resolved", events, false).Modify(widget)

	widget.Modify(response)
	return response
}

func (e EventsWidget) BuildEventsTable(title string, events []models.Event, current bool) *gr.Element {
	count := 0
	maxArchive := 10
	...
	for i, event := range events {
		if current {
			...
			if event.Archive && count < maxArchive {
				models.ExtractAwsmTable(i, event, &header, &rows)
				count++
			}
		}
	...
	el.Header5(gr.Text(title)).Modify(response)
